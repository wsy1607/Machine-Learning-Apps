#this script creats a MongoDB collection 'broadLocation' using the location
#data from the local csv file: 'BroadLocation.csv'
...
#load location data
def loadlocation():
    print "loading the broad location info"
    data = []
...
def insertdb(data):
    for i,row in enumerate(data):
        ...
        if '' in row:
            ...
            k = len(row)
        db1.broadLocation.insert({"_id":str(ObjectId()),"locationName":row[0],"locationList":row[1:k]})
