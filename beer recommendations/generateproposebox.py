#this script generates proposed beer boxes for beer club members
...
def getboxusers():
    boxUsersInfoList = []
    for proposeBoxUser in db1.proposeBox.find({"toMachineLearn":True}):
        boxUserDict = {}
...
def checkinputs(inventoryInfo,beersData,users):
    #check inventoryId
...
if __name__ == '__main__':
    ...
    users = getboxusers()
    #check all inputs
    checkinputs(inventoryInfo,beersData,users)
    #get initial recommended beers
    print "working on getting proposed beer boxes"
