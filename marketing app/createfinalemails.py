...
#create final emails
def insertfinalemails(approvedEmailList):
...
#update version info to final emails
def updatefinalemails(versionRatio):
...
if __name__ == '__main__':
    ...
    #we would like to have 100 final emails for each round for now
    n = 100
    #load the propose email list
    proposeEmailList = loadproposeemails()
    ...
    #update central emails
    updateemails(approvedEmailList,rejectedEmailList)
    #load version info
    versionRatio = loadversions()
    #update final emails with version info
    updatefinalemails(versionRatio)
